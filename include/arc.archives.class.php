...
class Archives
{
    ...
    function GetPreNext($gtype='')
    {
        ...
        if(count($this->PreNext)<2)
        {
            ...
            else
            {
                ...
                $this->PreNext['prename'] = "None"; //上一篇文章标
            ...
            else
            {
                ...
                $this->PreNext['nextname'] = "None"; //下一篇文章标题
    ...
    function ReplaceKeyword($kw,&$body)
    {
        ...
        foreach ($matches[0] as $key => $value) {
            ...
            $item = str_replace($matches[1][$key][0], md5($value[0]."ITBIGOIBGUALKVNOA6VITK31"), $value[0]);
        ...
        while ($row = $this->dsql->GetArray()) {
            ...
            $keywords[$key]['rpurl'] = $key_url;
        }

        global $cfg_replace_total, $cfg_replace_sort;
        if ($cfg_replace_total > 0) {
            foreach ($keywords as $key => $value) {
                if (substr_count(strtolower($body), strtolower($key)) <= 0) {
                    unset($keywords[$key]);
                }
            }
    
            // 随机:0 顺序:1
            if($cfg_replace_sort == 0) {
                uksort($keywords, function() { return rand() > rand(); });
            }
    
            // 替换总数限制
            $keywords = array_slice($keywords, 0, $cfg_replace_total);
        }

        foreach ($keywords as $keyword => $keyword_value) {
            preg_match_all("#{$keyword}#i", $body, $matches);
            $keyword_url = trim($keyword_value['rpurl']);

            foreach ($matches[0] as $key => $value) {
                $item = "<a href='$keyword_url' target='_blank'><u>$value</u></a>";
                $replaces[$value]['item'] = $item;
                $replaces[$value]['item_md5'] = md5($item);
        ...
        foreach ($replaces as $key => $value) {
            ...
            if($cfg_replace_num > 0) {
                $body = preg_replace("#{$key}#", $value['item_md5'], $body, $cfg_replace_num);
            } else {
                $body = str_replace($key, $value['item_md5'], $body);
            }
        }

        foreach ($replaces as $key => $value) {
            $body = str_replace($value['item_md5'], $value['item'], $body);
        }
